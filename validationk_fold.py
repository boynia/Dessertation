...
def s_phs(url_list, model, tokenizer, max_length):
    ...
    # Here, if any URL is predicted as phishing (assumed to be 1), we flag the page.
...
# Updated number of epochs to 5 for quick training; adjust as needed, for example to 100 as in the paper.
baseline_model.fit(X_train, y_train, epochs=100, batch_size=32, validation_data=(X_val, y_val))
...
# ----------------------------------------------
# 4-Fold Cross Validation (K4 CV)l
# ----------------------------------------------
...
for train_index, test_index in kf.split(X, y):
    ...
    # Build a new model instance for each fold to ensure independent training
    model_cv = build_baseline_model(vocab_size)
    
    # Train on the current fold; using a validation_split from training data
    history = model_cv.fit(X_train_cv, y_train_cv, epochs=100, batch_size=32, 
...
# Run the unit tests (if running as main)
